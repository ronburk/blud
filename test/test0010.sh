...
cleanup()
{
    rm -f "$log" "$direct_log" "$private_log" "$public_log"
...
# --lua is the supported standalone LuaJIT test entry point.  This avoids
# requiring a compiler and keeps the test independent of the host shell.
BLUD_SCOPE_ENV=environment "$root/blud" --lua "$root/test/test0010.luatest"
BLUD_SCOPE_ENV=environment "$root/blud" -B -f "$root/test/test0010.blud" all target_parent >"$log"
BLUD_SCOPE_ENV=environment "$root/blud" -B -f "$root/test/test0010.blud" target_child >"$direct_log"
...
if "$root/blud" -f "$root/test/test0010.private.blud" >"$private_log" 2>&1; then
...
if "$root/blud" -f "$root/test/test0010.public.blud" >"$public_log" 2>&1; then
